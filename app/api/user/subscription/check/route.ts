        ...
        if (mpResponse.ok && data.results) {
            ...
            const activeSub = data.results.find((sub: any) => {
                ...
                if (createdAfter) {
                    ...
                    // 5 min buffer para reloj desincronizado
                    const threshold = new Date(createdAfter).getTime() - (5 * 60 * 1000);
                    const isNewEnough = subDate >= threshold;

                    if (!isNewEnough) {
                        console.log(`[Subscription Filter] Ignorando sub antigua ${sub.id} (${sub.date_created}) vs filtro (${new Date(threshold).toISOString()})`);
                    }
                    return isNewEnough;
                }
                return true;
            ...
            if (activeSub) {
                console.log(`[Subscription Success] Plan PRO activado para ${companyId}. Sub ID: ${activeSub.id}`);
