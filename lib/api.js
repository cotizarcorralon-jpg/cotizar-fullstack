...
export const getOrCreateCompany = async (userId) => {
    const res = await fetch(`${API_URL}/user/company?userId=${userId}`, { cache: 'no-store' });
    return res.json();
};

export const checkSubscriptionStatus = async (companyId, createdAfter = null) => {
    const res = await fetch(`/api/user/subscription/check`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ companyId, createdAfter })
    });
    return res.json();
};
